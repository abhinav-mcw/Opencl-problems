...
int main(int argc, char *argv[])
{
   ...
   // unsigned char outputData = new unsigned char[R * C * 2];
    std::vector<unsigned char> outputData(vec.size() * 2); 
...
//--------------------------------------------------------------------------------
    ...
    try
    {
        ...
        cl::Program program(context, util::loadProgram("mirrorHorizontal_kernel.cl"), true);
    ...
    //    d_input = cl::Buffer(umatImage.handle(), CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, image.total() * image.elemSize());
       ...
       //   d_input = cl::Buffer(context, vec.begin(), vec.end(), true);
        ...
        // Create a cl::Buffer for output 
          d_output = cl::Buffer(context, CL_MEM_WRITE_ONLY, sizeof(unsigned char) * vec.size()*2); 

      // Create the kernel functor
 
        cl::make_kernel<cl::Buffer, cl::Buffer,int, int> horizontal_mirror_image(program, "horizontal_mirror_image");
    ...
    // Execute the kernel
        ...
        horizontal_mirror_image(cl::EnqueueArgs(queue, global),
    ...
    cv::Mat mirrorImage(image.rows, image.cols * 2, CV_8UC1, outputData.data()); 

    std::cout << "mirrorImage matrix size: \n rows: " << mirrorImage.rows <<", cols: "<< mirrorImage.cols <<"; type: "<< mirrorImage.type() <<", "<<  std::endl;
    std::cout << "mirrorImage matrix dimension : " << mirrorImage.dims<< std::endl; std::cout << "mirrorImage total : " << mirrorImage.total() << std::endl; 
    std::cout << "mirrorImage elem size :" << mirrorImage.elemSize()<< std::endl;
    std::cout << "mirrorImage channels : "  << mirrorImage.channels()<< std::endl;
    std::cout << "mirrorImage size: " << mirrorImage.size()<< std::endl;

    
    // Display the original and rotated images
    cv::imshow("Original Image", image);
    cv::imshow("mirrored Image", mirrorImage);
    cv::imwrite("original.jpg",image);
    cv::imwrite("horizontal_mirror.jpg",mirrorImage);
    cv::waitKey(0);
